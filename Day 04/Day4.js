...
function createXmasTree(height) {
    ...
    var tree = [];
    var Line_length = 2 * height - 1;
    for (let i = 1; i <= height; i++) {
        tree.push(`${'_'.repeat((Line_length - (2 * i - 1))/2)}${'*'.repeat(2 * i - 1)}${'_'.repeat((Line_length - (2 * i - 1))/2)}\n`);
    }
    for (let i = 1; i <= 2; i++) {
        if (i == 1) {
            tree.push(`${'_'.repeat((Line_length - 1)/2)}#${'_'.repeat((Line_length - 1)/2)}\n`);
        } else {
            tree.push(`${'_'.repeat((Line_length - 1)/2)}#${'_'.repeat((Line_length - 1)/2)}`);
        }
    }
